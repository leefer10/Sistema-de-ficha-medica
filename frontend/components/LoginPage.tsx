...
export function LoginPage({ onNavigate }: LoginPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Crear URLSearchParams para OAuth2PasswordRequestForm
      ...
      // Extraer y guardar user_id del JWT
      const userId = ApiClient.extractUserId();
      if (userId) {
        localStorage.setItem("userId", userId.toString());
      }

      // Redirigir a welcome (onboarding)
      onNavigate?.("/welcome");
    } catch (err) {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                type="button"
                className="py-3 px-4 border rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
              >
